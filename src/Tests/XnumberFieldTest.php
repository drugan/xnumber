...
namespace Drupal\xnumber\Tests;
...
class XnumberFieldTest extends WebTestBase {
   ...
   * @var array
   */
  public static $modules = ['node', 'entity_test', 'field_ui', 'help', 'filter', 'readmehelp', 'xnumber'];
  ...
  public function testNumberIntegerField() {
    // Create a field to validate. The step setting is passed in order to skip
    // random settings' generation while creating the field because those
    // settings wiil be generated and saved with the widget.
    $field = $this->createNumberField('xinteger');

    $field_name = $field->getName();
    $storage = $field->getFieldStorageDefinition();
    $base_settings = $field->getItemDefinition()->getSettings();
    // Override base settings on the default form mode widget.
    ...
    $field->setSettings(['step' => '4444', 'min' => '-17776', 'max' => '26664']);
    $field->save();
  ...
  public function testNumberDecimalField() {
    ...
    $field = $this->createNumberField('xdecimal', ['precision' => 8, 'scale' => 4]);
    $field_name = $field->getName();
  ...
  public function testNumberFloatField() {
    ...
    // test entities, although on nodes in testNumberFormatter() works as
  ...
  public function testNumberFormatter() {
    ...
    $random_float = (string) rand(0, pow(10, 6));
    $random_integer = (string) rand(0, pow(10, 6));
  ...
  public function testCreateNumberFloatField() {
    ...
    $field = $this->createNumberField('xfloat', [], ['min' => '0']);
  ...
  public function testCreateNumberDecimalField() {
    ...
    $field = $this->createNumberField('xdecimal', [], ['min' => '0']);

    // Set the minimum value to a decimal value.
    $this->assertSetMinimumValue($field, 0.01);
    // Set the minimum value to an integer value.
    $this->assertSetMinimumValue($field, 1);
  }
  ...
  private function getRandomNumberSettings($field) {
    ...
    $ceil = 9999;
    $off = NUMBER_FIELD_TEST_DECIMALS_OFFSET;
    ...
    if ($type == 'xinteger') {
      ...
      while ($tmp_max >= $valid) {
        ...
        $max = $valid;
        $valid = bcadd($valid, $step);
      }
      while ($default_value == $value && $default_value >= $min) {
        $default_value = $valid = bcsub($valid, $step);
      }
    }
    elseif ($type == 'xdecimal' || $type == 'xfloat') {
  ...
  private function saveNumberFormDisplaySettings($field, $field_name, $test_settings = []) {
    ...
    // prepared by getRandomNumberSettings() for a "user" to insert in field.
    $widget_settings['value'] = $settings['value'];
    $widget_settings['field_name'] = $field_name;
    $widget_settings['default_value'] = is_numeric($widget_settings['default_value']) ? $widget_settings['default_value'] : '';
 ...
 */
if (!defined('NUMBER_FIELD_TEST_DECIMALS_OFFSET')) {
  define('NUMBER_FIELD_TEST_DECIMALS_OFFSET', 3);
}
